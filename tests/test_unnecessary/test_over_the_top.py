...
def test_chainlink_api_call_without_link(
    job_id, dev_account, node_account, chainlink_fee
):
    # Arrange
...
@pytest.mark.skip(reason="overkill testing")
def test_chainlink_api_call_with_link_fulfilled(
    job_id, dev_account, node_account, chainlink_fee, expiry_time
):
    # Arrange
    link_token = LinkToken.deploy({"from": dev_account})
    oracle = MockOracle.deploy(link_token.address, {"from": dev_account})
